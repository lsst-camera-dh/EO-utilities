...
from lsst.eo_utils.base.butler_utils import getButler
...
from .file_utils import flat_summary_tablename, flat_summary_plotname,\
    raft_flat_tablename, raft_flat_plotname

from .analysis import FlatAnalysisBySlot, FlatAnalysisFunc

from .meta_analysis import FlatSummaryByRaft, FlatTableAnalysisByRaft,\
    FlatSummaryAnalysisFunc


class ptc_analysis(FlatAnalysisFunc):
    """Class to extract the photon transfer curve"""
    
    argnames = STANDARD_SLOT_ARGS + ['bias', 'superbias', 'rafts']
    iteratorClass = FlatAnalysisBySlot
    stat_ctrl = afwMath.StatisticsControl()

    def __init__(self):
        """C'tor"""
        FlatAnalysisFunc.__init__(self, "ptc")
    ...
    def extract(butler, data, **kwargs):
        ...
        ptd_data = dict()
        for i in range(1,17):
            ptd_data['AMP%02i_MEAN' % (i+1)] = []
            ptd_data['AMP%02i_VAR' % (i+1)] = []
        ...
        for id_1, id_2 in zip(flat_files[::2], flat_files[1::2]):
            ...
            for i, amp in enumerate(amps):
                ...
                image_1 = unbias_amp(im_1, serial_oscan, bias_type='spline', superbias_im=superbias_im, region=imaging)
                image_2 = unbias_amp(im_2, serial_oscan, bias_type='spline', superbias_im=superbias_im, region=imaging)

                fratio_im = afwImage.ImageF(image_1, True)
                operator.itruediv(fratio_im, image_2)
                fratio = mean(fratio_im)
                image_2 *= fratio
                fmean = (mean(image_1) + mean(image_2))/2.

                fdiff = afwImage.ImageF(image_1, True)
                fdiff -= image_2
                fvar = var(fdiff)/2.

                ptd_data['AMP%02i_MEAN' % (i+1)].append(fmean)
                ptd_data['AMP%02i_VAR' % (i+1)].append(fvar)
        ...
        dtables.make_datatable('ptc', ptc_data[key])
...
class ptc_stats(FlatAnalysisFunc):
    """Class to analyze the overscan flat as a function of row number"""

    argnames = STANDARD_SLOT_ARGS + ['rafts']
    iteratorClass = FlatTableAnalysisByRaft
    tablename_func = raft_flat_tablename
    plotname_func = raft_flat_plotname

    def __init__(self):
        """C'tor """
        FlatAnalysisFunc.__init__(self, "_ptc_stats")
    ...
    def plot_fits(dtables, figs):
        ...
        idx = 0
        for islot, slot in enumerate(["S11"]):
    ...
    def plot(dtables, figs):
        ...
        @param fgs (FigureDict)       Keeps track of the figures
        """
        ptc_stats.plot_fits(dtables, figs)
...
class ptc_summary(FlatSummaryAnalysisFunc):
    """Class to analyze the overscan flat as a function of row number"""

    argnames = ['dataset', 'butler_repo', 'skip', 'plot']
    iteratorClass = FlatSummaryByRaft
    tablename_func = flat_summary_tablename
    plotname_func = flat_summary_plotname

    def __init__(self):
        """C'tor"""
        FlatSummaryAnalysisFunc.__init__(self, "_ptc_sum")
